...
public:
    ...
    {
        changes->write_log("\n\t_GraspCommand() just called. start time: ");
	changes->init_timer();
	changes->init_ptimer();
	...
	#ifdef GRASPCOMMAND_TIMING
	changes->end_timing_session(" setting parameters");
	#endif
	...
	#ifdef GRASPCOMMAND_TIMING
	changes->end_timing_session(" determining the final hand pose");
	#endif
	
	/**** Jackson's interface ***/
	if (!changes->interface.check_hand_position(ptraj)){
		changes->end_timing_session("Checking grasp distance failure.");
		#ifdef GRASPCOMMAND_TIMING
		changes->write_log("\n\tEnding time: ");
		changes->print_wall_clock_time();
		#endif
	...
	#ifdef GRASPCOMMAND_TIMING
	changes->end_timing_session("checking grasp distance ");
	#endif
        ...
        if( bComputeStableContacts ) {
			...
			#ifdef RECORD_CONTACTS
			temp_ss.str("");
			temp_ss << "Adding contact between " << _report->plink1->GetName() << " and " << _report->plink2->GetName() << " position: " << itcontact->pos[0] << " " << itcontact->pos[1] << " " << itcontact->pos[2] << "\n";
			string msg = temp_ss.str();
			changes->write_log(msg);
			#endif
                    }
                }
            }
	...
	/**JC addition: *****/
	#ifdef GRASPCOMMAND_TIMING
	changes->end_timing_session(" the determination of contacts on line __LINE__ ");
	#endif
	...
	#ifdef GRASPCOMMAND_TIMING
	changes->end_timing_session(" pumping the results to grasper module");
	#endif
		...
		analysis = _AnalyzeContacts3D_jc(c);
	...
	/****JC mod:***/
	#ifdef GRASPCOMMAND_TIMING
	changes->end_timing_session(" the analysis of contacts __LINE__");
	#endif
	...
	changes->end_timing_session(" setting the robot's controller");
	changes->write_log("\tFinished grasping process. End time: ");
	changes->print_wall_clock_time();
	changes->write_log("\n\n");

	//halt_for_input("How does that volume look?");
	cout << "\tGrasp volume: " << analysis.volume << endl;
...
protected:
    ...
    virtual GRASPANALYSIS _AnalyzeContacts3D_jc(const vector<CollisionReport::CONTACT>& contacts)
    {
	GRASPANALYSIS out_analysis;
	out_analysis.mindist = 0;
	if (contacts.size() < 4) {
		changes->write_log("need at least 4 points of closure to get a volume.\nAnd given any hand, that's an error.\n");
		out_analysis.volume = 0;
		return out_analysis;
	}

	//Convert the contacts to a convex hull usable format.
	std::vector<double> pts(contacts.size() * 3), vconvexplanes;
	for (unsigned int i = 0; i < contacts.size(); ++i) {
		pts[3*i] = contacts[i].pos.x;
		pts[3*i + 1] = contacts[i].pos.y;
		pts[3*i + 2] = contacts[i].pos.z;
	}

	//Compute 3D volume
	out_analysis.volume = _ComputeConvexHull(pts, vconvexplanes, boost::shared_ptr< vector<int> >(), 3);
	temp_ss.str("");
	temp_ss << "\tQHull gives a volume of " << out_analysis.volume << "\n";
	changes->write_log(temp_ss.str());
	temp_ss.str("");
    ...
    {
        /* Original, appears to think you need 7 3d points for a closure...
	if( contacts.size() < 7 ) {
            RAVELOG_DEBUG("need at least 7 contact wrenches to have force closure in 3D\n");
            return GRASPANALYSIS();
        }*/
        if( contacts.size() < 4 ) {
            RAVELOG_DEBUG("need at least 4 points of closure to get a volume\n");
            changes->write_log("need at least 4 points of closure to get a volume\n");
            return GRASPANALYSIS();
        }

        RAVELOG_DEBUG(str(boost::format("analyzing %d contacts for force closure\n")%contacts.size()));
        changes->write_log(str(boost::format("analyzing %d contacts for force closure\n")%contacts.size()));
	...
	#ifdef CONVEX_HULL_TIMING
	changes->end_timing_session(" filling contact vector");
	#endif
	...
	#ifdef CONVEX_HULL_TIMING
	changes->end_timing_session(" calculating convex hull");
	#endif
	...
	#ifdef CONVEX_HULL_TIMING
	changes->end_timing_session(" finished graspAnalysis function");
	#endif
...
#ifdef QHULL_FOUND
	temp_ss.str("");
	temp_ss << vpoints.size() << "\n";
	string num_pts = temp_ss.str();
	changes->write_log("\tNum points for convex hull: " + num_pts);
        ...
        return 0;
	this is a symtax error. It should prevent us from compiling without qhull
#endif
    }
